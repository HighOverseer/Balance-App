...
class TransactionRecordListFragment extends StatefulWidget {
  TransactionRecordListFragment(
      this.transactionRecords, this.navigateToAdd,
      {super.key, required this.selectedMonth, required this.changeMonths}
      );
      ...
      children: [
        Container(
            ...
            decoration: BoxDecoration(
                border:
                    Border(bottom: BorderSide(width: 3, color: colorsNavy))),
            child: Stack(
              children: [
                Row(
            ...
            child: Stack(
              children: [
                ListView(
                    ...
                    shrinkWrap: true,
                    children: widget.transactionRecords
                        .map((item) => GestureDetector(
                            onTap: () =>
                                widget.navigateToAdd(transactionRecord: item),
                            child: TranasactionRecordRow(
                              (fileUri){
                                setState(() {
                                  int index = widget.transactionRecords.indexOf(item);
                                  widget.transactionRecords[index].paymentReceiptFileUri = fileUri;
                                });
                              },
